...
async function runTests() {
  ...
  console.log(`- Unverified login rejection: ${unverifiedLogin.status === 403 ? 'PASS (HTTP 403 Forbidden)' : 'FAIL'}`);
  ...
  console.log(`- Post-verification login: ${loginRes.status === 200 ? 'SUCCESS' : 'FAIL'} (User: ${loginRes.data?.user?.name})`);
  ...
  console.log(`- Razorpay order creation: ${rpOrderRes.status === 200 ? 'SUCCESS' : 'FAIL'} (id: ${rpOrderRes.data?.id})`);
  ...
  if (rpKeySecret) {
    ...
    validSignature = crypto.createHmac('sha256', rpKeySecret).update(signaturePayload).digest('hex');
  }
  ...
  console.log(`- Order creation & payment confirmation: HTTP ${orderRes.status} (Order ID: ${orderRes.data?.orderId})`);
  ...
  console.log(`- '${thinCrustAfter?.name}' stock after order: ${thinCrustAfter?.stock_qty} (Decremented by 1: ${baselineQty - 1 === thinCrustAfter?.stock_qty})`);

  results['TEST 1'] = orderRes.status === 201 && baselineQty - 1 === thinCrustAfter?.stock_qty ? 'PASS' : 'FAIL';
  ...
  console.log(`- Admin login status: HTTP ${adminLoginRes.status} (Role: ${adminLoginRes.data?.user?.role})`);
  ...
  if (thinCrustAfter) {
    ...
    console.log(`- Update threshold to 15: HTTP ${updateThresh.status} (New threshold: ${updateThresh.data?.ingredient?.low_stock_threshold})`);
    ...
    console.log(`- Update quantity to 60: HTTP ${updateStock.status} (New stock: ${updateStock.data?.ingredient?.stock_qty})`);
    stockUpdateOk = updateStock.status === 200;
  }
  results['TEST 2'] = adminLoginRes.status === 200 && thresholdUpdateOk && stockUpdateOk ? 'PASS' : 'FAIL';
  ...
  for (const st of statuses) {
    ...
    if (updateRes.status !== 200) {
      ...
      console.log(`- Transition to '${st}' failed: HTTP ${updateRes.status} (${updateRes.data?.message})`);
    } else {
  ...
  console.log(`- Insufficient inventory order response: HTTP ${excessiveOrder.status} (${excessiveOrder.data?.message})`);
  ...
  console.log(`- Stock remaining: ${postCheckIng?.stock_qty} (Negative prevented: ${postCheckIng?.stock_qty >= 0})`);
  ...
  results['TEST 4'] = excessiveOrder.status === 400 && postCheckIng?.stock_qty >= 0 ? 'PASS' : 'FAIL';
  ...
  console.log(`- Normal user accessing /api/orders/admin/all: HTTP ${adminApiAttempt.status} (${adminApiAttempt.data?.message})`);
  ...
  console.log(`- Normal user updating inventory: HTTP ${adminInvAttempt.status} (${adminInvAttempt.data?.message})`);

  results['TEST 5'] = adminApiAttempt.status === 403 && adminInvAttempt.status === 403 ? 'PASS' : 'FAIL';
  ...
  console.log(`- Forged signature order response: HTTP ${forgedPayment.status} (${forgedPayment.data?.message})`);
  ...
  results['TEST 7'] = failedAttempt.status === 400 && cheeseQtyPre === cheesePost?.stock_qty ? 'PASS' : 'FAIL';
